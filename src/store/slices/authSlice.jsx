...
const API_URL = `${(import.meta.env.VITE_API_URL || 'https://ink-sphere-backend-ukrx.vercel.app')}/api/auth`;
...
export const completeProfile = createAsyncThunk('auth/completeProfile', async (formData, thunkAPI) => {
  try {
    const response = await authAxios().put(`${(import.meta.env.VITE_API_URL || 'https://ink-sphere-backend-ukrx.vercel.app')}/api/users/complete-profile`, formData, {
      headers: {
...
export const updateProfile = createAsyncThunk('auth/updateProfile', async (formData, thunkAPI) => {
  try {
    const response = await authAxios().put(`${(import.meta.env.VITE_API_URL || 'https://ink-sphere-backend-ukrx.vercel.app')}/api/users/profile`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        Authorization: `Bearer ${localStorage.getItem('token')}`
      }
