...
export const ForgotPasswordModal = ({ isOpen, onClose }) => {
  ...
  const handleSendOtp = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${(import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000')}/api/auth/forgot-password`, {
        method: 'POST',
  ...
  const handleResetPassword = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${(import.meta.env.VITE_API_URL || 'http://127.0.0.1:5000')}/api/auth/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
