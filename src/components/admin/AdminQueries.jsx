...
export const AdminQueries = () => {
  ...
  const fetchQueries = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`${(import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/queries/admin`, {
  ...
  const handleResolve = async (queryId) => {
    ...
    try {
      ...
      await axios.put(
        `${(import.meta.env.VITE_API_URL || 'http://localhost:5000')}/api/queries/admin/${queryId}/resolve`,
        { reply: replyText },
        { headers: { Authorization: `Bearer ${adminToken}` } }
      );
