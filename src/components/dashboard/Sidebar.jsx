...
export const Sidebar = ({ isMobileOpen, setIsMobileOpen }) => {
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col justify-between p-6 bg-zinc-950 text-zinc-100 border-r border-zinc-800/80">
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center text-center pt-2">
          <div className="relative mb-3 group">
            <div className="h-20 w-20 rounded-full p-1 bg-gradient-to-tr from-amber-500 to-zinc-700 shadow-md">
              <img
                src={user?.avatar || 'https://via.placeholder.com/150'}
                alt={user?.name || 'User Avatar'}
                className="h-full w-full rounded-full object-cover border-2 border-zinc-900"
              />
            </div>
            <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full bg-emerald-500 border-2 border-zinc-900" />
          </div>

          <h3 className="font-bold text-white text-base tracking-wide truncate max-w-full">
            {user?.name || 'Guest User'}
          </h3>
          <p className="text-xs text-amber-400 font-medium truncate max-w-full mt-0.5">
      ...
      <div className="pt-4 border-t border-zinc-800/80">
        <button
          ...
          className="w-full flex items-center gap-3.5 rounded-xl px-3.5 py-3 text-sm font-medium text-red-400 hover:bg-red-500/10 transition-all duration-200 group cursor-pointer"
        >
